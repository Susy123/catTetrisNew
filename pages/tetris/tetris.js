...
var HISTORY_SCORE = 'history';
var blockItemWidth = 25;
...
Block.prototype = {
  printBlock: function () {
    ...
    for (var i = 0; i < 4; i++) {
      for (var j = 0; j < 4; j++) {
        if (this.shape[i][j] == 1) {
          console.log('allblock',allBlock[i + this.pos[0]][j + this.pos[1]])
          if (allBlock[i + this.pos[0]][j + this.pos[1]] && this.dir == 39) {
          // if (z_index[i + this.pos[0]][j + this.pos[1]] == 1 && this.dir == 39) {
            this.pos[1]--;
          } else if (allBlock[i + this.pos[0]][j + this.pos[1]] && this.dir == 37) {
...
function clearBlockNow(blockNow) {
  for (var i = 0; i < 4; i++) {
    for (var j = 0; j < 4; j++) {
      blockNow.shape[i][j] = 0;
    }
  }
}
//擦除上一时刻方块
...
Page({
  ...
  getScore: function () {
    ...
    // console.log('high score:', this.data.highScore)
    if(this.data.score>this.data.highScore){
      this.setData({
        highScore: this.data.score
      });
      wx.setStorageSync(HISTORY_SCORE, this.data.score);
    }
  },
   ...
   */
  onShow: function () {
    var historyScore = wx.getStorageSync(HISTORY_SCORE);
    if (!historyScore) {
      wx.setStorageSync(HISTORY_SCORE, 0);
    } else {
      this.setData({
        highScore: historyScore
      })
    }
  },
